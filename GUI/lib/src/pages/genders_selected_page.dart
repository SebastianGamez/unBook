...
class GendersSelectedPage extends StatefulWidget {
  const GendersSelectedPage({super.key});

  @override
  State<GendersSelectedPage> createState() => _GendersSelectedPageState();
}

class _GendersSelectedPageState extends State<GendersSelectedPage> {
  ...
  Future<List<String>> _getGenders() async {
    var preferences = await SharedPreferences.getInstance();
    if (preferences.getStringList('genders') != null) {
      return preferences.getStringList('genders')!;
    }
    return [];
  }
  ...
  void _getBooks() async {
    List genders = await _getGenders();

    genders.map((gender) async {
      Uri url = Uri.parse(
          'https://web-production-f5c8.up.railway.app/api/books/gender/$gender');

      var response = await http.get(url);
      List result = jsonDecode(response.body);

      setState(() => _books += result.map((book) => BookModel.fromJson(book)).toList());
    }).toList();
  }
}
